...
class ET_Builder_Module_Sign extends ET_Builder_Module {
	function init() {
		$this->name = esc_html__( 'Sign Now', 'et_builder' );
		$this->slug = 'et_pb_assineja';
		$this->child_slug      = 'et_pb_sign_field_assineja';
		...
		$this->main_css_element = '%%order_class%%.et_pb_assineja_form_container';
	...
	function predefined_child_modules() {
		...
		$output = sprintf(
				'[et_pb_sign_field_assineja field_title="%1$s" field_type="input" field_id="Titulo" required_mark="on" fullwidth_field="off" /]',
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$success_message = '' !== $success_message ? $success_message : esc_html__( 'Thanks for sign in', 'et_builder' );
		...
		if ( ! $et_contact_error && $nonce_result ) {
			...
			$post_id = assinejaCreateTopic( $my_post );
...
new ET_Builder_Module_Sign;

class ET_Builder_Module_Sign_item extends ET_Builder_Module {
	function init() {
		$this->name            = esc_html__( 'Field', 'et_builder' );
		$this->slug            = 'et_pb_sign_field_assineja';
		$this->type            = 'child';
...
new ET_Builder_Module_Sign_item;